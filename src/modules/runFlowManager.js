...
var RunFlowManager = (function () {
  ...
  function handleNextRoom(playerid, arg) {
    ...
    try {
      ...
      // Use RoomManager (now pure) to determine the step; fall back if unavailable
      var result = (typeof RoomManager !== 'undefined' &&
        ...
        : (typeof StateManager.advanceRoom === 'function'
            ? StateManager.advanceRoom(playerid, { scrip: 20, fse: 1 })
            : null);
      ...
      // Announce first-clear bonus (if any) — UI belongs in RunFlow
      if (result.firstClearBonusFSE && result.firstClearBonusFSE > 0) {
        whisperPanel(playerid, 'First Clear Bonus',
          '✪ +' + result.firstClearBonusFSE + ' FSE for defeating your first boss!'
        );
      }

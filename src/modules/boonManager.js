...
var BoonManager = (function () {
  ...
  function cardHTML(c, i){
    ...
    var head =
      '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px;">' +
        ...
        '<span style="padding:2px 6px;border-radius:6px;font-size:11px;font-weight:700;'
          +'background:'+st.badgeBg+';color:#fff;letter-spacing:0.3px;">'+ _.escape(r) +'</span>' +
      '</div>';
    ...
    return (
      '<div style="border:1px solid '+st.br+';background:'+st.bg+';'
        +'padding:10px 10px 12px 10px;border-radius:8px;margin:10px 0;">'
        + head + body + btn +
      '</div>'
    );
  }
  ...
  function renderOfferCards(playerName, ancestor, cards, freeMode){
    ...
    if (typeof sendDirect === 'function') {
      ...
      var shell = (typeof UIManager !== 'undefined' && UIManager.panel)

...
var RoomManager = (function () {
  ...
  function announceShop(playerid, phase, roomNumber) {
    ...
    if (roomNumber === 3) {
      whisperText(playerid, '🛒 Shop Available — use <b>!openshop</b> when you are ready.');
    }
    if (roomNumber === 5) {
      whisperText(playerid, '🛒 Optional Shop unlocked (use <b>!openshop</b> as desired).');
    }
  }
  ...
  function advance(playerid, opts) {
    ...
    if (stage === 'pre-room') {
      ...
      var body;
      if (type === 'boss') {
        body = '👑 The final chamber awaits.<br>Run the encounter, then use <b>!nextroom</b> to claim your rewards.';
      } else {
        body = '⚔️ Room ' + nextRoom + ' Ready.<br>Resolve the encounter, then use <b>!nextroom</b> to collect rewards.';
      }
    ...
    if (!baseResult || baseResult.firstEntry) {
      ...
      whisperPanel(playerid, 'Room ' + nextRoom + ' Ready', '⚔️ Proceed into the chamber and use <b>!nextroom</b> after the fight.');
      announceShop(playerid, 'enter', nextRoom);
      return {
        status: 'ready',
        type: type,
